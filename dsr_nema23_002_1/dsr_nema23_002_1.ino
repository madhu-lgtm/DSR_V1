/*
Code version2.1:- 
Code Version2 :- Add Encoder Knob for Controlling the Position 
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(en_clk_pin),step_cw,RISING);
...
void loop() {
  ...
  //req_step = map(knob_step,0,KSPR,0,SPR);

    if (cond1 == HIGH)
    {
      for(int n = 0; n < map_knob_stepper; n++){
